...
from app.modules.conversations.schemas import MessageCreate, MessageFromWebhook
from app.modules.conversations.models import MessageType, MessageDirection
from app.modules.leads.service import LeadService
from app.modules.leads.schemas import LeadUpdate, LeadCreate
...
class N8NWebhookHandler:
    ...
    def handle_message_received(
        ...
        *,
        tenant_id: uuid.UUID,
        data: N8NMessageReceived,
    ) -> dict:
        """
        Registra mensagem RECEBIDA do cliente.
        Retorna dados para notificação WebSocket.
        """
        from app.modules.conversations.exceptions import DuplicateMessageError

        # Monta os dados da mensagem para o webhook
        message_data = MessageFromWebhook(
            external_id=data.external_id or f"n8n_{uuid.uuid4().hex[:12]}",
            phone=data.phone,
            content=data.content,
            message_type=MessageType(data.message_type) if data.message_type else MessageType.TEXT,
        ...
        try:
            ...
            return {"success": False, "error": "Mensagem duplicada"}

        # Retorna dados para WebSocket
        return {
            "success": True,
            "conversation_id": str(conversation.id),
            "message_id": str(message.id),
            "message": {
                ...
                "message_type": data.message_type or "text",
                "created_at": message.created_at.isoformat(),
                "sender_name": data.name or data.phone,
    ...
    def dispatch(
        ...
        *,
        ...
        handlers = {
            ...
            "message_received": (N8NMessageReceived, N8NWebhookHandler.handle_message_received),
